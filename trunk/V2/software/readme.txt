USAGE:
unzip the project root folder on the SD-card.
navigate \software\csv folder
change the actually voice-text on csv folder (notepad.exe)
set the default text to speech voice on the control panel.

running the .cmd script
doube click en-stock_open9x_emartee.cmd

the script generate emartee-voice module compatible wav-s on the sd-card root folder.
This wavs usage directly on emartee voice module.
--------------------------------------------------------------------------------------------------
g:\> cscript.exe tts.vbs en-stock_open9x.csv 100 0  [codec] [AddSilentMsec]
 parameter1= volume (0-100)
 parameter2= speed (0~5)
[ parameter3= [e] | ad4 | high ]   (e=emartee 8khz 8bit pcm default codec)
[ end silent in msec]

 or doube click en-stock_open9x_emartee.cmd
 
 or doube click en-stock_er9x.cmd

This code generate wavs or AD4 from csv on root folder. functional root directory of the sd-card
with default (actual TTS voice)
Demo 30 days professional voices
http://www.kobaspeech.com/downloads.html
--------------------------------------------------------------------------------------------------
...
output conversion 16Khz/8bit .wav (ematree module)
